...
@app.route("/")
def home():
    return "Broadway API is live. Try /broadway"
...
def get_broadway():
    ...
    try:
        ...
        print("⚠️ Live scrape failed, using cache if available:", e)
        if is_cache_valid():
            return jsonify(load_cache())
        return jsonify({"error": "Unable to retrieve Broadway data"}), 500